...
import be.kdg.teamh.exceptions.*;
...
public class CirkelsessieController {
    ...
    public String gastToken(@PathVariable int id, @RequestParam(name = "t") String token)
            throws GebruikerNotFound, IsForbidden, DeelnameNotFound, CirkelsessieNotFound {
        if (auth.isGuest(token) && service.find(id) != null && mayAccessCirkelsessie(token, id))
            return token;
        throw new IsForbidden();
    }
    ...
    public void createKaart(@PathVariable("id") int id, @RequestHeader(name = "Authorization") String token, @RequestBody Kaart kaart) throws CirkelsessieNotFound, GebruikerNotFound, IsForbidden, DeelnameNotFound {
        if (!auth.isRegistered(token) && !mayAccessCirkelsessie(token, id)) {
            throw new IsForbidden();
        }
    ...
    private boolean mayAccessCirkelsessie(String token, int id) throws GebruikerNotFound, DeelnameNotFound, CirkelsessieNotFound {
        Gebruiker gast = auth.findByToken(token);
        List<Deelname> deelnames = gast.getDeelnames();
        return deelnames.contains(deelnameService.findByCirkelsessie(id));
    }
}
